...
shared_examples "with basic date scopes" do |date_column = :created_at |
  ...
  describe "date scopes" do
    ...
    let!(:last_week_obj) { FactoryGirl.create test_factory, date_column.to_sym => 1.week.ago }
    let!(:yesterday_obj) { FactoryGirl.create test_factory, date_column.to_sym => 24.hours.ago }
    let!(:hour_ago_obj) { FactoryGirl.create test_factory, date_column.to_sym => 1.hour.ago }
    let!(:five_minute_ago_obj) { FactoryGirl.create test_factory, date_column.to_sym => 5.minutes.ago }
    let!(:after_midnight_ago_obj) { FactoryGirl.create test_factory, date_column.to_sym => Date.today.midnight + 5.minutes }
    let!(:before_midnight_ago_obj) { FactoryGirl.create test_factory, date_column.to_sym => Date.today.midnight - 5.minutes }
